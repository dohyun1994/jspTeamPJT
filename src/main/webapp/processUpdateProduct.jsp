...
<%
	...
	if (rs.next()) {		
		if (fileName != null) {
			sql = "UPDATE food SET p_name=?, p_unitPrice=?, p_description=?, p_category=?, p_unitsInStock=?, p_fileName=? WHERE p_id=?";	
			pstmt = conn.prepareStatement(sql);
			...
			sql = "UPDATE food SET p_name=?, p_unitPrice=?, p_description=?, p_category=?, p_unitsInStock=? WHERE p_id=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, name);
			pstmt.setInt(2, price);
			pstmt.setString(3, description);
